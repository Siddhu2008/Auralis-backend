    ...
    if name not in _MODELS:
        ...
        if os.path.exists(path):
            try:
                ...
                logger.error(f'[Agent] Failed to load model {name}: {e}')
                _MODELS[name] = None
        else:
...
def classify_intent(text):
    ...
    try:
        ...
        label = intent_engine.predict_intent(text)
        return label, 0.9
    except Exception as e:
        logger.error(f'[Agent] classify_intent error: {e}')
        return 'unknown', 0.0
...
def _call_ai_service(prompt, model='grok-beta', timeout=10):
    """Call the unified AI service. Primary: Grok-beta, Fallback: Gemini-1.5-flash."""
    try:
        # Try requested model (Grok)
        result = ai_service.generate_content(prompt, model=model)
        if result:
            return result
        
        # Fallback to Gemini if Grok fails or key is missing
        if model != 'gemini-1.5-flash':
            logger.info(f'[Agent] Fallback to Gemini for prompt snippet: {prompt[:50]}...')
            return ai_service.generate_content(prompt, model='gemini-1.5-flash')
            
        return None
    except Exception as e:
        logger.error(f'[Agent] AI Service error: {e}')
        return None
...
Return ONLY the summary."""
    summary = _call_ai_service(prompt) or "Meeting concluded successfully."
...
Keep it professional."""
    ...
    script = _call_ai_service(prompt) or "Ready for video summary."
    return script
...
Provide a concise, helpful answer:"""
    ...
    answer = _call_ai_service(prompt, timeout=10)
    if not answer:
...
Keep the email concise and professional."""
    ...
    result = _call_ai_service(prompt, timeout=10)
    if result:
        try:
...
Auralis Response:"""
    ...
    response = _call_ai_service(prompt, timeout=10)
    if not response:
        from utils.ai_service_unified import ai_service
