...
def create_schedule(user_id, title, start_time, participants=None, duration_minutes=None, request_timezone=None):
    ...
    # Generate unique access code and join link
    access_code = ''.join(random.choices(string.ascii_uppercase + string.digits, k=6))
    # Ensure uniqueness (simple check for now)
    while Schedule.query.filter_by(access_code=access_code).first():
        access_code = ''.join(random.choices(string.ascii_uppercase + string.digits, k=6))
    
    join_link = f"/meeting/{access_code}"
    ...
    try:
        ...
        # settings already fetched above in create_schedule logic (line 37)
        if settings and settings.notifications_enabled:
            create_notification(user_id, f"Meeting scheduled: {title}", type='schedule')
    except Exception as e:
        print(f"[create_schedule] Notification Error: {e}")
