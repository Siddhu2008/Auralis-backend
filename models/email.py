...
class Email(db.Model):
    ...
    direction = db.Column(
        db.String(20), default="outgoing", nullable=False
    )  # outgoing/incoming/draft
    category = db.Column(
...
def create_email_entry(
    ...
    row = Email(
        ...
        approved=approved,
        unread=True,  # Default new emails to unread
    )
    db.session.add(row)
    db.session.commit()
    return row.to_dict()
...
def get_email_metrics(user_id):
    ...
    unread = Email.query.filter_by(
        user_id=user_id, unread=True, is_deleted=False
    ).count()
    urgent = Email.query.filter_by(user_id=user_id, category="urgent").count()
    outgoing = Email.query.filter_by(
        user_id=user_id, direction="outgoing"
    ).count()
    return {
        "total": total,
        "unread": unread,
