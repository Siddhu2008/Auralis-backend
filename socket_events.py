...
_last_proxy_time = {} # Track proxy generation cooldown: sid -> timestamp
...
def register_socket_events(socketio):
    ...
    def handle_join_user(data):
        ...
        if user_id:
            ...
            print(f"[WebSocket] User {user_id} joined private notification room")
    ...
    def handle_join_room(data):
        ...
        if is_returning_host:
            ...
            # Demote interim host if any (ensure only one host)
            old_host_sid = room_hosts.get(room)
            if old_host_sid and old_host_sid != request.sid:
                emit('role_assigned', {'role': 'participant'}, to=old_host_sid)
                print(f"DEBUG: Interim host {old_host_sid} demoted to participant.")
    ...
    def handle_end_meeting_for_all(data):
        ...
        if room: room = room.strip()
        host_sid = room_hosts.get(room)
        print(f"DEBUG: end_meeting_for_all request from {request.sid} for room {room}. Current Host SID: {host_sid}")
        if host_sid != request.sid: 
            print(f"WARNING: Unauthorized end_meeting_for_all from {request.sid}")
            return
    ...
    def handle_end_meeting(data):
        ...
        if not transcript_text:
            ...
            emit('processing_status', {'status': 'ended', 'summary': "No content recorded."}, to=room)
            return

        # Prepare Meeting Log
        try:
            summary_text = "Meeting session completed."
    ...
    def handle_proxy_check(data):
        ...
        for sid, details in room_user_details.get(room, {}).items():
            if sid != sender_sid and proxy_states.get(sid):
                # Rate limit proxy to 1 response per 30 seconds per user
                last_time = _last_proxy_time.get(sid, 0)
                if now - last_time < 30:
                    continue
                ...
                # Fetch real user settings for the proxy
                user_profile = "A professional participant."
                if user_id:
                    try:
                        from models.user_settings import get_or_create_user_settings
                        settings = get_or_create_user_settings(user_id)
                        user_profile = f"Tone: {settings.assistant_tone}. Working Hours: {settings.working_hours_start}-{settings.working_hours_end}. "
                    except Exception:
                        pass
                
                from utils.ai_response import generate_proxy_response
                # In a real app, we'd fetch user_profile from DB
                response = generate_proxy_response(user_name, user_profile, message, transcript)
                
                if response:
                    _last_proxy_time[sid] = time.time()
                    proxy_msg = f"[Digital Twin] {response}"
                    emit('chat_message', {
                        'sender': sid,
                        'message': proxy_msg,
                        'timestamp': datetime.utcnow().isoformat()
