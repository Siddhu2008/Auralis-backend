...
@meeting_bp.route("/instant", methods=["POST"])
def instant_meeting():
    ...
    title = (data.get("title") or "Instant Meeting").strip()
    
    try:
        # Create a meeting starting NOW (with 1-minute buffer to avoid "past date" validation errors)
        now = datetime.utcnow()
...
@meeting_bp.route("/join/<access_code>", methods=["GET"])
def resolve_join_code(access_code):
    payload, err = _require_auth()
    if err:
        return err
    
    meeting = find_meeting_by_code_or_id(access_code)
    if not meeting:
        return jsonify({"error": "Invalid meeting code"}), 404
...
def list_past_meetings():
    ...
    meetings = (
        MeetingV2.query.filter(
            (MeetingV2.user_id == user_id) & (MeetingV2.status == "ended")
        )
        .order_by(MeetingV2.ended_at.desc())
        .all()
    )
    response = []
...
def list_upcoming_meetings():
    ...
    # Fetch meetings that are scheduled for today or later, or are currently live
    meetings = (
        MeetingV2.query.filter(
            (MeetingV2.user_id == user_id) & 
            (MeetingV2.status.in_(["scheduled", "live", "active"])) &
            ((MeetingV2.scheduled_start_at >= start_of_today) | (MeetingV2.status == "live"))
        )
...
@meeting_bp.route("/proxy/start", methods=["POST"])
def start_proxy():
    payload, err = _require_auth()
    if err:
        return err
    ...
    meeting = find_meeting_by_code_or_id(str(meeting_id))
    if not meeting:
        return jsonify({"error": "Meeting not found"}), 404
        
    user_id = int(payload["user_id"])
    
    # In a real system, this would spin up a headless browser or WebRTC client
    # For now, simulate proxy startup via sockets
    from utils.socket_instance import socketio
    if socketio:
        socketio.emit('agent_status', {
            'message': f"Digital Twin Agent proxying meeting: {meeting.title}",
            'type': 'proxy_active'
        }, room=f"user_{user_id}")

    return jsonify({"status": "active", "message": "Proxy Agent deployed", "meeting_id": meeting.id}), 200
